...
static bool SetNextChannel( TimerTime_t* time )
{
    ...
    else
    {
        if( delayTx > 0 )
        {
#if (NEWLAND_ACCELERATE_DEBUG != false)
			*time = 0;//0ms,加快发送速度
#else
			// Delay transmission due to AggregatedTimeOff or to a band time off
			*time = nextTxDelay;
#endif //(NEWLAND_ACCELERATE_DEBUG != false)          
            return true;
        }
...
static bool RxWindowSetup( uint32_t freq, int8_t datarate, uint32_t bandwidth, uint16_t timeout, bool rxContinuous )
{
    ...
    if( Radio.GetStatus( ) == RF_IDLE )
    {
        ...
        else
        {
            ...
            Radio.SetRxConfig( modem, bandwidth, downlinkDatarate, 1, 0, 8, timeout, false, 0, true/*false*/, 0, 0, false/*true*/, rxContinuous );
#else
...
LoRaMacStatus_t SendFrameOnChannel( ChannelParams_t channel )
{
...
#if (NEWLAND_USE_RX_TX_RF_SET != false)
       Radio.SetTxConfig( MODEM_LORA, 20/*txPower*/, 0, 1, datarate, 1, 8, false, true, 0, 0, false, 3e3 );
#else
    ...
    { // Normal LoRa channel
        Radio.SetMaxPayloadLength( MODEM_LORA, LoRaMacBufferPktLen );
#if (NEWLAND_USE_RX_TX_RF_SET != false)
        Radio.SetTxConfig( MODEM_LORA, 20/*txPower*/, 0, 0, datarate, 1, 8, false, true, 0, 0, false, 3e3 );
#else
