...
static void OnUsart2TimerEvent( void )
{
    ...
    //...
	if(F_USART2_RX_RECEIVING)//正在接收串口数据
	{
		//串口接收完成或结束
...
{
	for(uint8_t i=0; i<USART2_REC_LEN; i++)//接收缓冲,最大USART2_REC_LEN个字节
	{
	...
	//F_USART2_RX_RECEIVING=false;//串口接收正在进行中标记,1:进行中，0：暂停或结束
}

/******************************************************************************
*函数名称：void USART2_Process(void)
*功能：串口2进程处理,用于启动串口接收定时
*输入：无
*输出：无
*返回：无
******************************************************************************/
